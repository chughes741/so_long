...
- (void) scrollWheel:(NSEvent *)theEvent
{
	...
	if (sens > 0.2) button = 4;
	if (sens < -0.2) button = 5;
	
	sens = [theEvent deltaX];
	if (sens > 0.2) button = 6;
	if (sens < -0.2) button = 7;
...
{
	if (event_funct[12] != NULL) event_funct[12](event_param[12]);
}


- (void) closeNotification:(NSNotification *)note
{
	if (event_funct[17] != NULL) event_funct[17](event_param[17]);
}
...
// Generic hook system for all events
